...
*Where it is:* built and tested, all of it.

Two of the four turned out not to be middleware. robots and redirects each have
exactly one correct position, outside everything, so both are `downloader`
attributes that wrap the chain rather than plugins with a number somebody could
change. A redirect follower inside the robots check would fetch another host's
pages without ever reading its robots.txt, which is the whole argument in one
sentence.

RFC 9309 is implemented here rather than imported: being wrong is too permissive
on somebody else's site, under our name, and nothing in our own output looks
