...
This is a design for the API surface, not a description of the one that ships
today. It is the companion to [CLI.md](CLI.md) and uses the same five nouns, on
the principle that `scour job ls` and `GET /v1/jobs` should be the same
question asked twice. The last section maps every endpoint that exists now onto
