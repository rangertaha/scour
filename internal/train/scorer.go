...
// have.
func (t *Trainer) trainScorer(ctx context.Context, item *store.Item) (*ScoreResult, error) {
	obs, pos, neg, err := t.observations(ctx, item)
	if err != nil {
		return nil, err
	}
	...
	path := t.cfg.ScoreModelPath(item.Name)
...
func (t *Trainer) observations(ctx context.Context, item *store.Item) ([]bayes.Observation, int, int, error) {
	rows, err := t.store.FetchedURLs(ctx, item.ID)
	if err != nil {
		return nil, 0, 0, err
	}

	invalid, err := t.invalidURLs(ctx, item)
	if err != nil {
		return nil, 0, 0, err
	}
	...
	if classifier != nil {
		categories, t.classified = t.classifyPages(ctx, item, rows, classifier)
	}
...
func (t *Trainer) invalidURLs(ctx context.Context, item *store.Item) (map[string]bool, error) {
	rows, _, err := t.store.SearchRecords(ctx, item.ID, store.RecordQuery{})
	if err != nil {
		return nil, err
	}
...
// seedWords is what the model knows before it has seen anything: the item's
// name, its aliases, and the words in its property names and examples.
func seedWords(item *store.Item) []string {
	var out []string
	add := func(s string) {
...
// Scorer returns the scorer to crawl an item with: its trained model when
...
func Scorer(cfg config.Config, item *store.Item) (score.Scorer, bool, error) {
	...
	scorer, err := score.New(name, score.Config{
		Item:    item.Name,
		Seed:    seedWords(item),
		Path:    cfg.ScoreModelPath(item.Name),
