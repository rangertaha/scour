...
// driving scour is doing the same job a person does: describe an entity, crawl
...
func (s *Server) MCP() *mcp.Server {
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name: "list_entities",
		Description: "List the entities scour knows about and how many records " +
			"have been extracted for each.",
	}, s.mcpListEntities)

	mcp.AddTool(srv, &mcp.Tool{
		Name: "get_entity",
		Description: "Describe one entity: its aliases, properties, crawl targets " +
			"and content types.",
	}, s.mcpGetEntity)

	mcp.AddTool(srv, &mcp.Tool{
		Name: "add_entity",
		Description: "Create an entity or add to an existing one. Every field is " +
	...
	mcp.AddTool(srv, &mcp.Tool{
		...
		Description: "List the built-in schemas that can be passed as the template " +
			"argument to add_entity. A template fills in a kind of record's usual " +
			"properties, the words a page might label them with, and an example of each.",
	}, s.mcpTemplates)

	mcp.AddTool(srv, &mcp.Tool{
		Name: "crawl",
		Description: "Start crawling an entity's targets. Returns a job id " +
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "list_rules",
		Description: "List the extraction rules learned for an entity, with how often each fires.",
	}, s.mcpRules)

	mcp.AddTool(srv, &mcp.Tool{
		Name:        "search_records",
		Description: "Search the records extracted for an entity.",
	}, s.mcpSearch)
...
type listEntitiesOut struct {
	Entities []store.EntitySummary `json:"entities"`
}

func (s *Server) mcpListEntities(ctx context.Context, _ *mcp.CallToolRequest, _ struct{}) (*mcp.CallToolResult, listEntitiesOut, error) {
	rows, err := s.store.Entities(ctx)
	if err != nil {
	...
	if len(names) == 0 {
		return text("No entities yet. Use add_entity to define one."), listEntitiesOut{}, nil
	}
	return text("%s", strings.Join(names, ", ")), listEntitiesOut{Entities: rows}, nil
}
...
func (s *Server) mcpGetEntity(ctx context.Context, _ *mcp.CallToolRequest, in entityName) (*mcp.CallToolResult, getEntityOut, error) {
	entity, err := s.store.EntityFull(ctx, in.Name)
	if err != nil {
		return nil, getEntityOut{}, err
	}
	...
	return text("%s: %d targets, %d aliases, properties: %s",
			entity.Name, len(entity.Targets), len(entity.Aliases),
			strings.Join(props, ", ")),
		getEntityOut{Entity: entity}, nil
}

type addEntityIn struct {
	Name     string   `json:"name" jsonschema:"the entity's name"`
	Aliases  []string `json:"aliases,omitempty" jsonschema:"other words a page might use for this entity"`
...
type addEntityOut struct {
	Name    string   `json:"name"`
	Changes []string `json:"changes"`
}

func (s *Server) mcpAddEntity(ctx context.Context, _ *mcp.CallToolRequest, in addEntityIn) (*mcp.CallToolResult, addEntityOut, error) {
	if strings.TrimSpace(in.Name) == "" {
		return nil, addEntityOut{}, fmt.Errorf("name is required")
	}

	entity, err := s.store.CreateEntity(ctx, in.Name)
	if err != nil {
		return nil, addEntityOut{}, err
	}

	var changes []string
	if in.Template != "" {
		if err := applyTemplate(ctx, s.store, entity.ID, in.Template); err != nil {
			return nil, addEntityOut{}, err
		}
	...
	for _, d := range in.Domains {
		if err := s.store.AddTarget(ctx, entity.ID, store.TargetDomain, d, false, 0); err != nil {
			return nil, addEntityOut{}, err
		}
		changes = append(changes, "domain "+d)
	}
	for _, u := range in.URLs {
		if err := s.store.AddTarget(ctx, entity.ID, store.TargetURL, u, false, 0); err != nil {
			return nil, addEntityOut{}, err
		}
		changes = append(changes, "url "+u)
	}
	for _, p := range in.Props {
		if err := s.store.AddProperty(ctx, entity.ID, p.Name, p.Type, p.Example); err != nil {
			return nil, addEntityOut{}, err
		}
		changes = append(changes, "property "+p.Name)
	}

	out := addEntityOut{Name: entity.Name, Changes: changes}
	if len(changes) == 0 {
		return text("Entity %s exists; nothing to add.", entity.Name), out, nil
	}
	return text("%s: %s", entity.Name, strings.Join(changes, ", ")), out, nil
}
...
type crawlIn struct {
	Name     string `json:"name" jsonschema:"the entity to crawl"`
...
func (s *Server) mcpTrain(ctx context.Context, _ *mcp.CallToolRequest, in entityName) (*mcp.CallToolResult, jobOut, error) {
...
func (s *Server) mcpRules(ctx context.Context, _ *mcp.CallToolRequest, in entityName) (*mcp.CallToolResult, rulesOut, error) {
	entity, err := s.store.Entity(ctx, in.Name)
	if err != nil {
		return nil, rulesOut{}, err
	}

	rows, err := s.store.Rules(ctx, entity.ID)
	if err != nil {
		return nil, rulesOut{}, err
	}
	if len(rows) == 0 {
...
type searchIn struct {
	Name       string  `json:"name" jsonschema:"the entity to search"`
...
func (s *Server) mcpSearch(ctx context.Context, _ *mcp.CallToolRequest, in searchIn) (*mcp.CallToolResult, searchOut, error) {
	...
	rows, total, err := s.store.SearchRecords(ctx, entity.ID, store.RecordQuery{
...
type labelIn struct {
	Name  string `json:"name" jsonschema:"the entity the record belongs to"`
...
func (s *Server) mcpLabel(ctx context.Context, _ *mcp.CallToolRequest, in labelIn) (*mcp.CallToolResult, labelOut, error) {
	...
	n, err := s.store.LabelRecords(ctx, entity.ID, []uint{in.ID}, label)
	if err != nil {
		return nil, labelOut{}, err
	}
	if n == 0 {
