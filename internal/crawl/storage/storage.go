...
// Storage is a colly storage backed by the database, scoped to one item.
type Storage struct {
	ctx    context.Context
	store  *store.Store
	itemID uint
...
// New returns a storage for one item. The context bounds every query it
...
func (s *Storage) Visited(requestID uint64) error {
	return s.store.MarkVisited(s.ctx, s.itemID, requestID)
}

// IsVisited implements colly's storage.Storage.
func (s *Storage) IsVisited(requestID uint64) (bool, error) {
	return s.store.IsVisited(s.ctx, s.itemID, requestID)
}
...
func (s *Storage) Cookies(u *url.URL) string {
	cookies, err := s.store.Cookies(s.ctx, s.itemID, u.Host)
	if err != nil {
...
func (s *Storage) SetCookies(u *url.URL, cookies string) {
	if err := s.store.SetCookies(s.ctx, s.itemID, u.Host, cookies); err != nil {
